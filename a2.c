...
static void save_gray_frame(unsigned char *buf, int wrap, int xsize, int ysize, char *filename, AVFrame *pFrame);
...
static int decode_packet(AVPacket *pPacket, AVCodecContext *pCodecContext, AVFrame *pFrame)
{
  ...
  while (response >= 0)
  {
    ...
    if (response >= 0 && pCodecContext->frame_number == 3) {
      logging(
      ...
      if(pCodecContext->frame_number == framenumber_requested){
        save_gray_frame(pFrame->data[0], pFrame->linesize[0], pFrame->width, pFrame->height, frame_filename, pFrame);
      }
    }
  }
  return 0;
...
static void save_gray_frame(unsigned char *buf, int wrap, int xsize, int ysize, char *filename, AVFrame *pFrame)
{
  ...
  // Convert the frame from YUV to RGB
    ...
    printf("sizes frame: %d, %d\n", rgb_frame->linesize[0], pFrame->linesize[0]);
    FILE *fp = fopen("output.rgb", "wb");
    fwrite(rgb_frame->data[0], 1, pFrame->height * pFrame->width * 3, fp);
    fclose(fp);
